...
export class Logger extends EventEmitter<Logger.Events> implements Logger.Options {
    ...
    protected print(level: LogLevel, ...data: any[]): void {
        ...
        this.emit(LogLevel[level] as Logger.LogLevelKey, {
            ...options,
            pretty,
            simple
        });
    ...
    public emit<K extends string|symbol>(eventName: Key<K, Logger.Events>, ...args: Args<K, Logger.Events>): boolean {
...
export namespace Logger {
    ...
    export type Events = {
        [K in LogLevelKey]: [data: FormatterFormatOptions & { pretty: string; simple: string; }];
    }

    export type LogLevelKey = Exclude<keyof typeof LogLevel, number>;
}